...
def index():
    ...
    posts = db.paginate(query, page=page, per_page=main.config["POSTS_PER_PAGE"], error_out=False)
...
@login_required
@main.route('/post/<int:parent>/comment', methods=['GET', 'POST'])
...
@login_required
@main.route('/post/<int:parent>/like', methods=['GET', 'POST'])
def LikePost(parent):
    post = db.session.scalar(sa.select(Post).where(Post.id == parent))
